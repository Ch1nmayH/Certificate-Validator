...
const getUsers = async (req, res) => {
  try {
    ...
    if (req.query.isOrganization == "true") {
      const users = await user.find({ isOrganization: true });
      return res.status(200).json({ users });
    }

    if (req.query.isAdmin == "false" && req.query.isOrganization == "false") {
      const users = await user.find({ isAdmin: false, isOrganization: false });
      return res.status(200).json({ users });
    }
...
const getOrganization = async (req, res) => {
  try {
    const Organization = await user.find({ isOrganization: true });
...
const checkOrganizationAuth = async (req, res, next) => {
  try {
    const token = req.cookies.token;
    if (!token) {
      return res.status(401).json({ message: "You are not Authenticated" });
...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (existingUser.isOrganization) {
      return res
        .status(400)
